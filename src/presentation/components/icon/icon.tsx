import React from 'react'
import Styles from './icon-styles.scss'

export enum IconName {
  tumbUp = 'data:image/png;base64,',
  tumbDown = 'data:image/png;base64,',
}
type Props = {
  iconName: IconName
  className?: string
}

const Icon: React.FC<Props> = ({ iconName, className }: Props) => {
  const iconCollor = iconName === IconName.tumbDown ? Styles.red : Styles.green
  return (
    <div className={[Styles.iconWrap, iconCollor, className].join(' ')}>
      <img className={Styles.icon} src={iconName} />
    </div>
  )
}

export default Icon
